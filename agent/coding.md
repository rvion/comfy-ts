...
- **Cast whitelist** (each individually justified, do not add without updating this list):
  ...
  3. `globalThis as { comfyts?: ComfyTS }` — global registration: `src/state.ts` owns it, and `src/cli/serve/serveSettings.ts` probes the same global to find the settings path (family 11 extends this to the web entry's runtime probes).
  ...
  6. kind-discriminated `ComfyVar` narrowing (`sel as ToggleVar` after checking `kind`) — value-invariance blocks a clean union (`src/cli/tui/state/`, `src/cli/tui/imagePicker/`, `src/cli/tui/components/`, `src/cli/serve/`). This is the MANDATORY shape for every var branch: `instanceof` is false across the shipped bundles, so it is banned in `src/cli/` and `tests/var-kind-discrimination.test.ts` enforces it (architecture.md owns the WHY).
  ...
  9. serve payload/disk json validation: after every entry of a value is runtime-checked (`isLoraStrength`, key ∈ options, a shape probe), the record is claimed as its type — `src/cli/serve/applyVarPayload.ts`, plus the body and settings-file readers in `src/cli/serve/ServeApp.ts` and `src/cli/serve/serveSettings.ts`. Same wire-tolerance family as 4, but the check is COMPLETE, not soft.
  10. serve web ui wire tolerance (`src/cli/serve/web/`): the browser side of family 4 — jsonFetch's `as (T & { error?: string })` on api replies, the payload.ts/LorasControl normalizers (`asSeedForm`/`asSizeForm`/`asRecord`) shape-checking draft values before claiming them, the localStorage blobs in WebSt and EnhancerSt, and the llm.ts reply readers (models list, SSE deltas) which shape-check each field before keeping it — an external api drifts like ComfyUI does. Same reasoning: the json crossed a process boundary and is re-validated at the use site.
  11. runtime-boundary probes of the web entry (architecture item 13), each null-checked right after: `process.getBuiltinModule('node:fs')` results claimed as the structural NodeFsLike/NodeOsLike faces and node `readFileSync(p,'utf8')` as string (`src/storage/ComfyStorage.ts`); VARIABLE-specifier dynamic imports (typed as any by tsc) claimed as their module shapes (`src/host/ResilientWebsocket.ts` ws ctor, `src/utils/lazySharp.ts`); `globalThis` probes for WebSocket and the comfyts storage backref (`ResilientWebsocket.ts`, `ComfyStorage.ts` — extends family 3 beyond src/state.ts).
- Known violations to burn down (documented, do NOT replicate): scattered `as any` in `ComfyNode._convertPromptExtToPrompt` / `_writeDynamicCombo` / dynamic outputs wiring (one prompt-JSON shape, built dynamically from the schema — burn them down together, not one at a time); `softValidate`/`bong` returning lying casts on failure (both documented at the definition); manager generated unions claimed via `.as<T>()` without membership checks (live data may legitimately contain values newer than the generated union). (`ComfyRegistry` built-in `as any` seed burned down 2026-07-30: the pseudo-plugin is seeded into byTitle/byFile BEFORE codegen so its literals are union members.)
