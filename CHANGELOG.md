...
- **Lora strengths get a line each, with sliders**, and a lock that ties model and clip so one drag moves both. The lock starts on only when the two are already equal, so a lora deliberately tuned to 0.8/0.6 is not flattened by the first drag.
