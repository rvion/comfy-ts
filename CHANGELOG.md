...
- **The panel can be embedded, and the embedding page can talk to it.** Inside an `<iframe>` the panel speaks a small `postMessage` protocol (every message carries a `comfyTs` discriminator). Out: `ready` once booted, `selection` with `{module, draft}` on every change, and `result-action` when one of the host's buttons on a result card is pressed, carrying the image url, filename, index, module, draft, seeds and the prompt text at click time. In: `host-actions` with `[{id, label, title?}]`, which the panel draws on every result card and in the lightbox, and `set-prompt` with the text to put in the form's prompt var. `?prompt=<text>` on the panel url fills it at open. The host can also drive the panel: `set-selection` with `{module, draft}` switches workflow and draft (an unknown draft opens `default`, an unknown workflow changes nothing), `set-values` with `{values}` sets vars by name through the same rules as an edit in the form, and `get-state` asks for the form. The panel answers every selection change and every `set-values` with `state` carrying `{module, draft, values}`. Messages sent while the panel is still loading are applied once it is ready, in order. A plain tab sees none of this: nothing is drawn and no listener is installed unless there is a parent window.
