...
- **The lora card, redrawn**: uncropped preview, remove button back in its corner, the on/off switch beside the name, and ONE slider labelled `m+c` — click that label to set model and clip apart, click again to tie them. A lora whose two values already differ opens split. The sliders are drawn rather than left to the browser's default chrome, and a card drags from a grip so the sliders stay usable.
