...
- **The blur shortcut is now ⇧⌘B**, because ⌘B folds the menu column.
- **A menu column in the web panel.** Workspace (where `.comfy-ts/` lives), root (the folder serve scans), workflow, draft and host are stacked in a resizable left column. It opens with ☰ and the name, then plain sections (workspace, workflow, host, preview) instead of boxed cards. Fold it to an icon rail with ☰ or ⌘B, and the panel remembers its width and whether it is folded. Results placed on a side make three columns. On a phone the column is a drawer behind ☰, and a slim top bar names the workflow and the draft. The preview placement buttons (off, below, left, right, corner) are a `preview` card in the column, and the preview header keeps only what the preview shows. Workspace and root are read-only, and a click copies the full path. The index reply carries `workspace` and `root`.
