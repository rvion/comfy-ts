...
- **New keys: ⌘B folds the menu column, ⌘D duplicates the open draft, F2 renames it.** The blur moves to ⌘U. F2 is pressed on its own, as in the TUI; ⌘R stays the browser's reload.
- **A menu column in the web panel.** Workspace (where `.comfy-ts/` lives), root (the folder serve scans), workflow, draft and host are stacked in a resizable left column. It opens with ☰ and the name, then plain sections (workspace, workflow, draft, host, preview) instead of boxed cards. In the draft section, `duplicate this draft` and `new draft from defaults` create an auto-named draft and open it at once (a trailing number goes up, `shot 007` becomes `shot 008`, and a name without one gets `-2`), and a rename row does what F2 does. Fold it to an icon rail with ☰ or ⌘B, and the panel remembers its width and whether it is folded. Results placed on a side make three columns. On a phone the column is a drawer behind ☰, and a slim top bar names the workflow and the draft. The preview placement buttons (off, below, left, right, corner) are a `preview` section in the column, with the current placement in words under them, and the preview header keeps only what the preview shows. Workspace and root are read-only, and a click copies the full path. The index reply carries `workspace` and `root`.
- **Open drafts as tabs.** A tab bar over the work area lists every draft you opened, named by its draft (a name two workflows share also shows its workflow). A click jumps to it, × or a middle click closes it. ⌘1 to ⌘8 pick a tab, ⌘9 the last one, ⌘PageUp and ⌘PageDown step through them. A click on the open tab shows its name to edit, then duplicate, new default and delete. The serve process keeps the list (`GET` / `PUT /tabs`), so every window shows the same tabs, and `.comfy-ts/serve-tabs.json` keeps them across restarts.
- **Results by column count.** One number, 1 to 8, replaces the fit and grid buttons and the size slider: 1 draws each result as wide as the preview panel, 2 half, and so on. A result is its image alone; its title and delete button show over it on hover.
- **One run row.** Run and a stop square side by side (red while a run can be stopped), then `Clear N queue` and `Erase N img`, each carrying the count it acts on. Blur is one toggle button with its key inside, and a blurred image shows that key over it. The blur no longer spreads past the image's edges.
- **A copy button beside each prompt preview** copies the whole text, negative lines included.
- **The console opens at its last line** and follows new lines while you stay at the bottom.
- **Separate lora image sizes** for the form and the loras popup.
- **Thin scrollbars** in the panel's colors, instead of the system's wide dark ones.
- **Prompt completion is opt in per workflow, lora words included.** A prompt without `tags` completes nothing, even when its loras add words.
- **`inc` and `dec` seeds step on every run of a queue.** A queue of runs under `inc` used to alternate between two seeds.
- **Audio plays in WebKit.** The server answers byte ranges (`206 Partial Content`) on every binary reply, and sends its length, so Safari and WebKit app windows play the audio outputs. Chrome already did.
...
- **A real prompt editor.** Comments are dim and italic, negative lines red, weights show their number. ⌘↑ / ⌘↓ (or ⌃↑ / ⌃↓) changes the weight of the selection or of the tag under the cursor by 0.05, and 1.0 removes the parentheses. ⌘/ comments lines, ⌘⇧- moves lines into the negative prompt, ⌥↑ / ⌥↓ moves lines. A tag written twice, a tag a lora already adds, and a tag in both prompts get a wavy underline that says why. An estimated token count shows under each box.
- **Booru tag completion, per workflow**: `v.prompt(text, { tags: url })` completes tags from a list, sorted by post count, with aliases. The url is a local path (relative to the workflow file) or an http(s) url, fetched once and cached under `.comfy-ts/cache/tags/`. comfy-ts ships no tag list. `{ url, underscores: true }` keeps `long_hair` as written, `artistPrefix: '@'` prefixes artist tags (Anima wants it). Literal parentheses in tags are escaped so ComfyUI does not read them as weights. The active loras' words are offered too. `weights: false` marks a model that ignores `(text:1.2)`: the editor underlines every weight instead.
- **`//` after text is a comment too**: `1girl, red hair // try blue` sends `1girl, red hair`. A url keeps its `//`.
...
- **History**: a history button under every prompt and beside the refiner's input lists every prompt you ran (or refined) since the page opened, newest first, with how long ago and from which workflow. Search by words, arrow through the list to see each full text, enter to restore it. It is kept in memory only.
...
- **The panel can be embedded, and the embedding page can talk to it.** Inside an `<iframe>` the panel speaks a small `postMessage` protocol with its parent window. Every message carries a `comfyTs` discriminator. Only the parent is listened to: a message from any other window is ignored. The parent's origin is pinned from its first message, and later messages from another origin are ignored. Out: `ready` once the panel has booted with a draft open, or `error` with `code: 'boot'` when it cannot boot (the workflow list failed, no workflows, the opening draft unreadable). `selection` with `{module, draft}` whenever the open draft changes. `state` with `{module, draft, values}`, sent only once the parent has spoken: as the reply to a request, and without being asked after the user edits the form (on the autosave's debounce), switches draft, or a run writes its seeds back. `result-action` when one of the host's buttons on a result card is pressed, carrying the image url, filename, index, module, draft, seeds and the prompt text at click time. Before the parent has spoken, only `ready`, `selection` and the boot error go out. In: `host-actions` with `[{id, label, title?}]`, drawn on every result card and in the lightbox (an empty label or a repeated id is dropped). `set-prompt` with the text for the form's prompt var (the var declared `v.prompt`, else a text var named like a prompt that is not a negative prompt). `?prompt=<text>` on the panel url fills it at open. `set-selection` with `{module, draft}` switches workflow and draft (an unknown draft opens `default`). `set-values` with `{values}` sets vars by name through the same rules as an edit in the form. `get-state` asks for the form. Each `set-selection`, `set-values` and `get-state` gets exactly one reply: `state` with `request` naming it (a `set-values` reply also lists the names it did not apply in `rejected`), or `error` with `request` and a `code`: `unknown-module`, `select-failed`, `superseded` (the user switched away while it loaded) or `no-form`. Messages sent while the panel is still loading, or while it switches draft, wait and are applied in order. The message types ship as types from `comfy-ts/web`: `PanelToHost`, `HostToPanel`, `PanelState`, `PanelError`, `PanelResultAction`, `PanelHostAction`, `PanelRequest`. A plain tab sees none of this: nothing is drawn and no listener is installed unless there is a parent window.
...
- **You can watch the model write.** ComfyUI has a channel for text published DURING a node (a binary websocket frame, replacement semantics per node) that several nodes already use and core `TextGenerate` never does. It is wired through now: live text reaches `execution.progressText`, the `onProgress` callback, the cli run line, and the results panel, where the answer builds up as it is produced — the reasoning shown while it is the only thing there is, then the answer.
- **[`extra/comfyui-textgen-stream/`](extra/comfyui-textgen-stream/) is the ComfyUI node that makes `TextGenerate` use that channel.** Copy the folder into `ComfyUI/custom_nodes/` and restart. It wraps the per-token sampler for the duration of one call rather than reimplementing the sampling loop, so what it generates is identical to core; it just says so as it goes. `07-local-llm-text-gen` feature-detects it and falls back to core `TextGenerate` on a host without it.
- **The run card shows the executing node's own counter** either way — `TextGenerate 427/1024` tokens, `KSampler 12/20` steps — instead of only a global percent. Without the streaming node that counter is the whole live signal, since a text graph has no latent preview to fall back on.
...
- **How many loras the picker draws is yours to set, and the default is 200.** It stopped at 60 cards, which hides most of a real collection; the `… N more` line carries the number now, so you can take it further if your box and your collection are up to it. Kept in this browser, clamped to 1..2000 because every card is an image request. A browser still holding the old default of 60 picks up 200 once, since that value was stored whether or not anyone chose it.
- **The seed row is `=` `+` `?`, the value, and the dice.** `fixed / +1 / -1 / random` spelled out was a row of prose wider than the input it belongs to; the symbol is the mode, and the sentence lives in the tooltip. `-` is still a real mode a draft can hold, so it shows up only while it IS the mode and disappears once you pick another — the row never misreports the draft.
