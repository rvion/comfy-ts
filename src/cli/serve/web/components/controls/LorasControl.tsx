...
// strengths per active lora. Value shape = the POST payload contract:
// { "<name>": [model, clip] }, replaced by copy on every edit
...
export const LorasControl = observer(function LorasControl(p: { v: VarSt }) {
      ...
      <div className="loras-box">
         ...
         <div className="loras-list">
            {shown.map((name) => {
               ...
               return (
                  <div key={name} className={active ? 'lora-row active' : 'lora-row'}>
                     <input
                        type="checkbox"
                        checked={active}
