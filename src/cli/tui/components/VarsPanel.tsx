...
const VarRow = observer((p: { name: string; varDef: AnyVar; selected: boolean; labelW: number; st: TuiSt }) => {
...
export const VarsPanel = observer((p: { st: TuiSt }) => {
   ...
   return (
      <Box borderStyle="round" borderColor={focused ? 'cyan' : 'gray'} paddingX={1} flexDirection="column">
         ...
         {s.entries.map(([name, varDef], ix) => (
            <VarRow key={name} name={name} varDef={varDef} selected={ix === s.selIx} labelW={labelW} st={s} />
         ))}
      </Box>
   )
})
