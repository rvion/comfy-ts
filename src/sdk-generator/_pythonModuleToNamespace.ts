export function pythonModuleToPrefix(pythonModule: string): string {
   ...
   // unknown module layout: fall back to a sanitized module prefix instead of
   // crashing the whole codegen because of one unusual custom node
   console.warn(`[comfy-ts] unexpected pythonModule "${pythonModule}", using it as prefix as-is`)
   return `${pythonModule}.`
}
