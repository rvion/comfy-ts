...
export const fireredI2i = host.defineWorkflow({
   ...
   build: async (b, vars, wf) => {
      ...
      const encode = (text: string) => b.TextEncodeQwenImageEditPlus({ prompt: text, clip, vae, image1: image })
      ...
      b.SaveImage({ images: b.VAEDecode({ samples, vae }), filename_prefix: 'comfy-ts-zoo/firered-i2i' })
   },
})
...
if (import.meta.main) {
   ...
   const execution = await fireredI2i.run({ log: true })
