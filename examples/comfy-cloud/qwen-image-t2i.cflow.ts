// qwen-image t2i on Comfy Cloud — Qwen-Image 20B fp8, AuraFlow-shift sampling
// source template: image_qwen_image.json (official workflow-templates)
// needs COMFY_CLOUD_API_KEY (https://cloud.comfy.org, paid tiers)
// run directly:  bun examples/comfy-cloud/qwen-image-t2i.cflow.ts ["prompt"] [seed]
import { v } from 'comfy-ts'
import { cloudHost, requireCloudKey } from './cloudHost.ts'

const host = await cloudHost()

export const qwenImageT2i = host.defineWorkflow({
   id: 'qwen-image-t2i',
   vars: {
      prompt: v.prompt(
         'a vibrant neon-lit Hong Kong street in the afternoon, 1980s style, overlapping Chinese and English signs, rain-washed pavement reflections, cinematic',
      ),
      seed: v.seed(42),
      // template defaults: 20 steps / cfg 4 (its optional 8-step Lightning lora branch ships off, dropped here)
      steps: v.int(20, { min: 1, max: 60 }),
      cfg: v.float(4, { min: 0, max: 30 }),
      size: v.size({ width: 1328, height: 1328 }),
   },
   build: (b, vars) => {
      const model = b.ModelSamplingAuraFlow({
         model: b.UNETLoader({ unet_name: 'qwen_image_fp8_e4m3fn.safetensors', weight_dtype: 'default' }),
         shift: 3.1,
      })
      const clip = b.CLIPLoader({ clip_name: 'qwen_2.5_vl_7b_fp8_scaled.safetensors', type: 'qwen_image' })
      const vae = b.VAELoader({ vae_name: 'qwen_image_vae.safetensors' })
      const samples = b.KSampler({
         model,
         positive: b.CLIPTextEncode({ clip, text: vars.prompt.positive }),
         negative: b.CLIPTextEncode({ clip, text: vars.prompt.negative }),
         latent_image: b.EmptySD3LatentImage({ width: vars.size.width, height: vars.size.height, batch_size: 1 }),
         seed: vars.seed,
         steps: vars.steps,
         cfg: vars.cfg,
         sampler_name: 'euler',
         scheduler: 'simple',
         denoise: 1,
      })
      b.SaveImage({ images: b.VAEDecode({ samples, vae }), filename_prefix: 'comfy-ts-zoo/qwen-image-t2i' })
   },
})

export default qwenImageT2i

// standalone run (skipped when another driver — e.g. the TUI — imports this module)
if (import.meta.main) {
   requireCloudKey()
   if (process.argv[2]) qwenImageT2i.vars.prompt.set(process.argv[2])
   if (process.argv[3]) qwenImageT2i.vars.seed.set(Number(process.argv[3]))
   const execution = await qwenImageT2i.run({ log: true })
   for (const img of execution.images) console.log(`🟢 ${img.absPath}`)
   host.disconnect()
}
