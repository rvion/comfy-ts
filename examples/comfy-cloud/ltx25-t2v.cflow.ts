// ltx25 t2v on Comfy Cloud — LTX-2.5 22b distilled, text to video WITH a generated soundtrack, 121 frames @ 24fps ≈ 5s
// source template: video_ltx2_5_t2v.json (official workflow-templates)
...
// run directly:  bun examples/comfy-cloud/ltx25-t2v.cflow.ts ["prompt"] [seed]
import { v } from 'comfy-ts'
import { cloudHost, requireCloudKey } from './cloudHost.ts'

const host = await cloudHost()

export const ltx25T2v = host.defineWorkflow({
   id: 'ltx25-t2v',
   vars: {
      // ONE prompt var: `- ` lines are the NEGATIVE prompt, `//` lines are comments
      prompt: v.prompt(
         'A close-up of an Arctic hunter\'s face, his eyes fixed straight ahead, narrowed and locked on something beyond the frame. Snow whips across his fur-lined hood, wind howls, and he whispers: "it\'s close now."\n- pc game, console game, video game, cartoon, childish, ugly',
      ),
      seed: v.seed(42),
   ...
   build: (b, vars) => {
      ...
      // pass 1: video + audio latents sampled together at half resolution
      const base = b.SamplerCustomAdvanced({
         ...
         latent_image: b.LTXVConcatAVLatent({
            video_latent: b.EmptyLTXVLatentVideo({
      ...
      // pass 2: x2 latent upscale of the video, then a short refine over the rejoined av latent
      const refined = b.SamplerCustomAdvanced({
         ...
         latent_image: b.LTXVConcatAVLatent({
            video_latent: b.LTXVLatentUpsampler({
      ...
      b.SaveVideo({
         ...
         filename_prefix: 'comfy-ts-zoo/ltx25-t2v',
         format: 'auto',
         codec: 'auto',
      })
   },
...
export default ltx25T2v

// standalone run (skipped when another driver — e.g. the TUI — imports this module)
if (import.meta.main) {
   requireCloudKey()
   if (process.argv[2]) ltx25T2v.vars.prompt.set(process.argv[2])
   if (process.argv[3]) ltx25T2v.vars.seed.set(Number(process.argv[3]))
   const execution = await ltx25T2v.run({ log: true })
   // SaveVideo outputs land host side (no auto-download for video yet)
   console.log(`🟢 ${execution.status}: video saved on the host under comfy-ts-zoo/ltx25-t2v`)
   host.disconnect()
}
